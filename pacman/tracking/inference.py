...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0: allPossible[p] = emissionModel[trueDistance] * beliefs[p]

        allPossible.normalize()
        if sum(allPossible.values()) > 0 :
            self.resample(allPossible)
        else:
            self.initializeUniformly(gameState)

    def resample(self, distribution):
        particles = []
        for i in range(self.numParticles):
            particles.append(util.sample(distribution))
